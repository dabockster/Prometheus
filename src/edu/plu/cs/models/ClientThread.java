...
public class ClientThread extends Thread{
     ...
     */
    public ClientThread(Client client, Socket socket){this.client = client; this.socket = socket;  this.open(); this.start(); }
    /**
     * open()
     * Open the input stream to read from socket
     */
    public void open(){try{streamIn = new DataInputStream(socket.getInputStream()); bReader = new BufferedReader(new InputStreamReader(streamIn));}catch(IOException ioe){System.out.println("Failed to Open, Motherfucker: "+ ioe);}}
    /**
     * close()
     * End inputStream
     */
